<#
.SYNOPSIS
    JW Timer - Lightweight multi-monitor countdown timer.

.DESCRIPTION
    JW Timer displays a transparent countdown overlay on a selected monitor.

    The user only needs to:
        1. Select the meeting start time.
        2. Select the destination display.
         . Optionally identify connected displays.
...
.NOTES
    ...
    Usage        : Internal / authorized use only.

    Copyright notice:
    This software and its source code are protected by applicable intellectual
    property laws. Unauthorized copying, modification, distribution,
    publication or commercial use is prohibited without prior authorization
...
# =====================================================================
$script:TimerMutex = New-Object System.Threading.Mutex($false, "JWTimer.SingleTimer")
$script:TimerMutexOwned = $false
if(-not $script:TimerMutex.WaitOne(0, $false)){
    [System.Windows.Forms.MessageBox]::Show("A JW Timer is already running.", $script:AppName, "OK", "Information")
    exit
}
...
public static class JWTimerConsole
{
...
try {
    ...
    [System.Diagnostics.Debug]::WriteLine(
        "JW Timer: Unable to initialize console interop. $($_.Exception.Message)"
    )
}

try {
    $consoleWindow = [JWTimerConsole]::GetConsoleWindow()
    if($consoleWindow -ne [IntPtr]::Zero){
        [JWTimerConsole]::ShowWindow($consoleWindow, 0) | Out-Null
    }
} catch {
    [System.Diagnostics.Debug]::WriteLine(
        "JW Timer: Unable to hide the console window. $($_.Exception.Message)"
    )
}
...
function Show-CountdownOverlay {
    ...
    # preserving the behavior of the original JW Timer implementation.
    #
...
$subtitleLabel.Text = "Simple countdown for meetings and presentations"
...
# $footerLabel.Text = "JW Timer v$($script:Version)  |  Developed by $($script:Developer)"
