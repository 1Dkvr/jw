...
.DESCRIPTION
    ...
    - If the selected meeting time has already passed today, the timer automatically targets the same time tomorrow.
    ...
    - If a selected display disappears or its display bounds change while the countdown is running, the current countdown session is closed cleanly.
    ...
    - The metadata structure is intended to be consumed by future release and update tooling.
...
# =====================================================================
$configPath = Join-Path -Path $PSScriptRoot -ChildPath "JwCountdown.config.psd1"

if(-not (Test-Path -LiteralPath $configPath -PathType Leaf)){
    [System.Windows.Forms.MessageBox]::Show(
        "The shared configuration file was not found.`r`n`r`n$configPath",
        "JW Countdown",
...
try {
    $script:Configuration = Import-PowerShellDataFile -LiteralPath $configPath -ErrorAction Stop
} catch {
    [System.Windows.Forms.MessageBox]::Show(
        ("Unable to load the shared configuration file.`r`n`r`n" + $_.Exception.Message),
        "JW Countdown",
        [System.Windows.Forms.MessageBoxButtons]::OK,
        [System.Windows.Forms.MessageBoxIcon]::Error
    ) | Out-Null
...
$script:AppName   = $script:Configuration.Project.Name
$script:Version   = $script:Configuration.Project.Version
$script:Developer = $script:Configuration.Project.Developer
