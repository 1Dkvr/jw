...
.NOTES
    ...
    Created      : 26.09.01
    Version      : Defined in application metadata
    Developer    : 1Dkvr
...
$script:TimerMutex = New-Object System.Threading.Mutex(
    $false,
    "JWCountdown.SingleTimer"
)
...
try {
    ...
    # ================================================================
    #
    # Selected monitors are stored by their index in the Screen[]
    # collection returned by Screen.AllScreens.
    #
    # Examples:
    #     [0]       = Display 1
    ...
    $ColorTextPrimary = [System.Drawing.Color]::FromArgb(
        30,
        35,
        45
    )

    $ColorTextSecondary = [System.Drawing.Color]::FromArgb(
        100,
        110,
        125
    )
    ...
    $ColorAccent = [System.Drawing.Color]::FromArgb(
        37,
        99,
        235
    )

    $ColorAccentLight = [System.Drawing.Color]::FromArgb(
        235,
        243,
        255
    )
...
'@
    ...
    try {
        ...
        [System.Diagnostics.Debug]::WriteLine(
            "[JW Countdown] Unable to initialize console interop."
        )
    }
    ...
    try {
        ...
        $consoleWindow =
            [JWCountdownConsole]::GetConsoleWindow()

        if($consoleWindow -ne [IntPtr]::Zero){

            [JWCountdownConsole]::ShowWindow(
                $consoleWindow,
                0
            ) | Out-Null
        }
    ...
    function Close-JWCountdownOverlays {
        ...
        try {
            ...
            [System.Diagnostics.Debug]::WriteLine(
                "[JW Countdown] Error while closing countdown overlays: $($_.Exception.Message)"
            )
        }
    }
    ...
    #
    # All overlays use ONE shared timer and ONE target time so every
    # display remains synchronized.
    #
    ...
    if($script:TimerMutexOwned){
        ...
        try {
            ...
            [System.Diagnostics.Debug]::WriteLine(
                "[JW Countdown] Unable to release the single-instance mutex."
            )
        }
    }
    ...
    try {
        ...
        [System.Diagnostics.Debug]::WriteLine(
            "[JW Countdown] Unable to dispose the single-instance mutex."
        )
    }
}
