...
# =====================================================================
#
# A named system-wide mutex prevents multiple instances of JW Countdown
# from running simultaneously.
#
# The mutex is intentionally owned for the complete lifetime of the
# application process and is released during the final cleanup stage.
#
...
try {
    if(-not $script:TimerMutex.WaitOne(0, $false)){
        [System.Windows.Forms.MessageBox]::Show("A JW Countdown is already running.", $script:AppName, "OK", "Information") | Out-Null
        return
    }
    ...
    # ================================================================
    #
    # The application now stores a collection of selected screen
    # indexes instead of a single selected monitor.
    #
    # Example:
    #     [0]       = Display 1
    #     [0,2]     = Displays 1 and 3
    #     [0,1,2]   = Displays 1, 2 and 3
    #
    $script:SelectedScreenIndexes = New-Object System.Collections.Generic.List[int]

    # Preserve the original behavior:
    # the first display is selected by default when available.
    #
    # The actual display list is initialized below.
    #
...
'@
    ...
    try {
        ...
        [System.Diagnostics.Debug]::WriteLine("[JW Countdown] Unable to hide the PowerShell console window.")
    }
    ...
    if($null -eq $screens -or $screens.Count -eq 0){
        [System.Windows.Forms.MessageBox]::Show("No display was detected.", $script:AppName, "OK", "Error") | Out-Null
        return
    }
    ...
    # primary monitor because Windows does not guarantee enumeration
    # order.
    #
    ...
    # Fallback in the unlikely case no primary screen is reported.
    #
    ...
    function Close-JWCountdownOverlays {
        ...
        # Prevent recursive close events from calling this method again.
        #
        ...
        try {
            ...
            [System.Diagnostics.Debug]::WriteLine("[JW Countdown] Error while closing countdown overlays: $($_.Exception.Message)")
        }
    }
    ...
    function Show-CountdownOverlays {
        ...
        foreach($display in $Displays){
            ...
            # The countdown text remains visible while the rest of the
            # overlay visually disappears into the desktop/application.
            #
        ...
        # --------------------------------------------------------------
        #
        # The current time is calculated once per tick and then applied
        # to every display label.
        #
        $timer.Add_Tick({
            $now = [datetime]::Now
        ...
        # --------------------------------------------------------------
        #
        # Do not wait one full second before showing the correct value.
        #
        ...
        try {
            ...
            #
            # The first overlay will later be used as the modal message
            # loop that keeps the entire WinForms application responsive.
            # ----------------------------------------------------------
            ...
            #
            # The other overlays remain visible and responsive because
            # they are part of the same WinForms UI thread.
            # ----------------------------------------------------------
    ...
    # ShowUpDown provides a compact native time selector and avoids
    # unnecessary calendar-style date interaction.
    #
    ...
    for($i = 0; $i -lt $screens.Count; $i++){
        ...
        #
        # This preserves the robust mechanism already used by the
        # original implementation and avoids relying on a closure
        # capturing the loop variable.
        # --------------------------------------------------------------
        $screenButton.Tag = $i

        # --------------------------------------------------------------
        # Toggle selection when the display card is clicked.
        # --------------------------------------------------------------
    ...
    $startButton.Add_Click({
        try {
            ...
            # ----------------------------------------------------------
            if($script:SelectedScreenIndexes.Count -eq 0){
                [System.Windows.Forms.MessageBox]::Show("Please select at least one destination display.", $script:AppName, "OK", "Warning") | Out-Null
                return
            }
            ...
            if($selectedScreens.Count -eq 0){
                [System.Windows.Forms.MessageBox]::Show("The selected displays are no longer available.", $script:AppName, "OK", "Warning") | Out-Null
                return
            }
            ...
            try {
                Show-CountdownOverlays -Displays $selectedScreens -TargetTime $targetTime
            } finally {
                # ------------------------------------------------------
                # Close the setup application when the countdown ends.
                #
                # This is intentional:
                # the application represents one countdown session.
                # Once that session ends, the process should terminate
                # and release the single-instance mutex.
                # ------------------------------------------------------
            ...
            [System.Windows.Forms.MessageBox]::Show("An unexpected error occurred.`r`n`r`n$($_.Exception.Message)", $script:AppName, "OK", "Error") | Out-Null
        }
    })
    ...
    # Pressing Enter starts the timer.
    #
    ...
    # =================================================================
    #
    # The mutex is released regardless of whether the application exits
    # normally or an unexpected exception occurs.
    #
